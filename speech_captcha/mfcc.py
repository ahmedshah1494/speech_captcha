...
#             print self.filters[leftfr:rightfr,whichfilt]
    ...
    def sig2sc(self, sig, mag_feats=False):
        nfr = int(len(sig) / self.fshift + 1)
        power = numpy.zeros((nfr, self.nfft/2 + 1))
        frames = numpy.zeros((nfr,self.wlen))
        fr = 0
        ...
        for fr in range(nfr):
            ...
            if len(frame) < self.wlen:
                ...
                frame[self.wlen:] = 0
            frames[fr] = frame
        frames = self.pre_emphasis_(frames) * self.win
        
        fft = numpy.fft.rfft(frames, self.nfft)
        # Square of absolute value
        power = fft.real * fft.real + fft.imag * fft.imag

        freqs = numpy.linspace(self.lowerf, self.upperf, num=power.shape[1])
        W = numpy.dot(freqs*power, self.filters).clip(1e-5,numpy.inf)
        if mag_feats:
            den = numpy.dot(freqs, self.filters).clip(1e-5,numpy.inf)
        else:
            den = numpy.dot(power, self.filters).clip(1e-5,numpy.inf)
        sc = W/den
        return sc

    def frame2sc(self, frame, mag_feats=False):
        frame = self.pre_emphasis(frame) * self.win
        fft = numpy.fft.rfft(frame, self.nfft)
        # Square of absolute value
        power = fft.real * fft.real + fft.imag * fft.imag
        freqs = numpy.linspace(self.lowerf, self.upperf, num=power.shape[0])
        W = numpy.dot(freqs*power, self.filters).clip(1e-5,numpy.inf)
        if mag_feats:
            den = numpy.dot(freqs, self.filters).clip(1e-5,numpy.inf)
        else:
    ...
    def pre_emphasis_(self, frames):
        shifted_frames = numpy.roll(frames,1,axis=1)
        shifted_frames[:,[0]] = numpy.roll(shifted_frames[:,[0]],1)
        shifted_frames[0,0] = 0
        shifted_frames *= self.alpha
        outfr = frames - shifted_frames
        return outfr
...
if __name__ == '__main__':
    m = MFCC()
    _,sig = wavfile.read('LDC2007S03.wav')
    scmc = m.sig2sc(sig, mag_feats=True)
    print scmc
